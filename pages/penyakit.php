...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'tambah') {
        $s = $conn->prepare("INSERT INTO penyakit (kode_penyakit,nama_penyakit,jenis,deskripsi) VALUES (?,?,?,?)");
        $s->bind_param("ssss", $kode,$nama,$jenis,$desk);
        if ($s->execute()) { $msg = "Penyakit berhasil ditambahkan."; }
        else { $msg = "Kode sudah digunakan."; $msg_type='error'; }
        $s->close();
    } elseif ($act === 'edit') {
        $id = (int)$_POST['id'];
        $s  = $conn->prepare("UPDATE penyakit SET kode_penyakit=?,nama_penyakit=?,jenis=?,deskripsi=? WHERE id=?");
        $s->bind_param("ssssi", $kode,$nama,$jenis,$desk,$id);
        $s->execute(); $s->close();
        $msg = "Data penyakit berhasil diperbarui.";
    } elseif ($act === 'hapus') {
        $id = (int)$_POST['id'];
        $conn->query("DELETE FROM penyakit WHERE id=$id");
        $msg = "Data penyakit berhasil dihapus."; $msg_type='error';
    }
    $jumlah_penyakit = $conn->query("SELECT COUNT(*) t FROM penyakit")->fetch_assoc()['t'];
}
...
if ($search) {
    ...
    $sql .= " WHERE nama_penyakit LIKE '%$esc%' OR kode_penyakit LIKE '%$esc%' OR jenis LIKE '%$esc%'";
}
$sql .= " ORDER BY kode_penyakit ASC";
$rows = $conn->query($sql);

layoutHead("Data Penyakit");
layoutBody();
...
<main class="lg:ml-52 min-h-screen p-4 lg:p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-bold text-[#1C1C2E]">Data Penyakit</h1>
            <p class="text-xs text-[#9B9AB0] mt-0.5">Kelola data penyakit ikan patin</p>
        </div>

        <button onclick="openModal('modalTambah'); getKodeOtomatis('penyakit', 'inputKodePenyakit')" class="btn-accent flex items-center gap-2">
            <?php icon('add') ?>
            Tambah
        </button>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-[#E8E7E1] overflow-hidden">
        ...
        <div class="flex items-center justify-between px-5 py-4 border-b border-[#F0EFE9]">
            <p class="text-sm font-semibold text-[#1C1C2E]">Total: <span class="text-[#7B7AFF]"><?= $jumlah_penyakit ?></span> penyakit</p>
            <form method="GET" class="flex items-center gap-2">
                <div class="relative">
                    <svg class="w-3.5 h-3.5 absolute left-3 top-1/2 -translate-y-1/2 text-[#9B9AB0]" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M9 3.5a5.5 5.5 0 100 11 5.5 5.5 0 000-11zM2 9a7 7 0 1112.452 4.391l3.328 3.329a.75.75 0 11-1.06 1.06l-3.329-3.328A7 7 0 012 9z" clip-rule="evenodd"/></svg>
                    <input type="text" name="q" value="<?= htmlspecialchars($search) ?>" placeholder="Cari penyakit..." class="input-base pl-9 py-2 w-full text-xs">
                </div>
                <button type="submit" class="btn-accent py-2 px-4 text-xs">Cari</button>
                <?php if ($search): ?><a href="penyakit.php" class="btn-ghost py-2 px-3 text-xs">Reset</a><?php endif; ?>
            </form>
        </div>
            ...
            <table class="w-full text-sm min-w-[900px]">
                <thead>
                    <tr class="text-[10px] uppercase tracking-wide text-[#9B9AB0] bg-[#FAFAF8]">
                        ...
                        <th class="text-left px-5 py-3 font-semibold">Nama Penyakit</th>
                ...
                <?php if ($rows->num_rows === 0): ?>
                    <tr><td colspan="6" class="text-center py-12 text-[#9B9AB0] text-sm">Tidak ada data ditemukan</td></tr>
                <?php else: while ($r = $rows->fetch_assoc()): 
                    $jenis_lower = strtolower($r['jenis']); ?>
                    <tr class="hover:bg-[#FAFAF8] transition-colors fade-in">
                        <td class="px-5 py-3 font-bold text-[#7B7AFF] text-xs"><?= htmlspecialchars($r['kode_penyakit']) ?></td>
                        <td class="px-5 py-3 font-semibold text-[#1C1C2E]"><?= htmlspecialchars($r['nama_penyakit']) ?></td>
                        <td class="px-5 py-3"><span class="pill pill-<?= $jenis_lower ?>"><?= $r['jenis'] ?></span></td>
                        <td class="px-5 py-3 text-xs text-[#9B9AB0] max-w-[200px] truncate"><?= htmlspecialchars($r['deskripsi']) ?></td>
                        <td class="px-5 py-3">
                            <div class="flex items-center justify-center gap-1.5">
                                ...
                                <button onclick="confirmHapus(<?= $r['id'] ?>, '<?= addslashes($r['nama_penyakit']) ?>')"
...
<div id="modalTambah" class="modal-backdrop">
    <div class="modal-card">
        <div class="flex items-center justify-between mb-5">
            <div>
                <h3 class="font-bold text-[#1C1C2E]">Tambah Penyakit</h3>
                <p class="text-xs text-[#9B9AB0] mt-0.5">Isi data penyakit ikan patin baru</p>
            </div>
        ...
        <form method="POST" class="space-y-3">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-semibold text-[#9B9AB0] mb-1">Kode</label>
                    <input type="text" name="kode_penyakit" id="inputKodePenyakit" required class="input-base">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-[#9B9AB0] mb-1">Jenis</label>
                    <select name="jenis" required class="input-base">
            ...
            <div>
                <label class="block text-xs font-semibold text-[#9B9AB0] mb-1">Nama Penyakit</label>
                <input type="text" name="nama_penyakit" placeholder="Nama penyakit" required class="input-base">
            </div>
            <div>
                <label class="block text-xs font-semibold text-[#9B9AB0] mb-1">Deskripsi</label>
                <textarea name="deskripsi" rows="2" placeholder="Deskripsi singkat..." class="input-base resize-none"></textarea>
            </div>
...
<div id="modalEdit" class="modal-backdrop">
    <div class="modal-card">
        <div class="flex items-center justify-between mb-5">
            <div>
                <h3 class="font-bold text-[#1C1C2E]">Edit Penyakit</h3>
                <p class="text-xs text-[#9B9AB0] mt-0.5">Perbarui data penyakit</p>
            </div>
        ...
        <form method="POST" class="space-y-3">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-semibold text-[#9B9AB0] mb-1">Kode</label>
                    <input type="text" name="kode_penyakit" id="editKode" required class="input-base">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-[#9B9AB0] mb-1">Jenis</label>
                    <select name="jenis" id="editJenis" required class="input-base">
                        <option value="Bakteri">Bakteri</option>
                        <option value="Virus">Virus</option>
                        <option value="Jamur">Jamur</option>
                        <option value="Parasit">Parasit</option>
            ...
            <div>
                <label class="block text-xs font-semibold text-[#9B9AB0] mb-1">Nama Penyakit</label>
                <input type="text" name="nama_penyakit" id="editNama" required class="input-base">
            </div>
            <div>
                <label class="block text-xs font-semibold text-[#9B9AB0] mb-1">Deskripsi</label>
                <textarea name="deskripsi" id="editDesk" rows="2" class="input-base resize-none"></textarea>
            </div>
            <div class="flex gap-2 pt-2">
...
<div id="modalHapus" class="modal-backdrop">
    <div class="modal-card max-w-sm text-center">
        ...
        <h3 class="font-bold text-[#1C1C2E] mb-1">Hapus Penyakit?</h3>
        <p class="text-xs text-[#9B9AB0] mb-5">Penyakit <strong id="hapusNama" class="text-[#1C1C2E]"></strong> akan dihapus permanen beserta seluruh basis aturannya.</p>
...
function openEdit(d) {
    ...
    document.getElementById('editKode').value = d.kode_penyakit;
    document.getElementById('editNama').value = d.nama_penyakit;
