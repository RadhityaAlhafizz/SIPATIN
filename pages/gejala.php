...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'tambah') {
        $s = $conn->prepare("INSERT INTO gejala (kode_gejala,nama_gejala) VALUES (?,?)");
        $s->bind_param("ss", $kode, $nama);
        if ($s->execute()) { $msg = "Gejala berhasil ditambahkan."; }
        else { $msg = "Kode sudah digunakan."; $msg_type='error'; }
        $s->close();
    } elseif ($act === 'edit') {
        $id = (int)$_POST['id'];
        $s  = $conn->prepare("UPDATE gejala SET kode_gejala=?,nama_gejala=? WHERE id=?");
        $s->bind_param("ssi", $kode,$nama,$id);
        $s->execute(); $s->close();
        $msg = "Data gejala berhasil diperbarui.";
    } elseif ($act === 'hapus') {
        $id = (int)$_POST['id'];
        $conn->query("DELETE FROM gejala WHERE id=$id");
        $msg = "Data gejala berhasil dihapus."; $msg_type='error';
    }
    $jumlah_gejala = $conn->query("SELECT COUNT(*) t FROM gejala")->fetch_assoc()['t'];
}
...
if ($search) {
    ...
    $sql .= " WHERE nama_gejala LIKE '%$esc%' OR kode_gejala LIKE '%$esc%'";
}
$sql .= " ORDER BY kode_gejala ASC";
$rows = $conn->query($sql);

layoutHead("Data Gejala");
layoutBody();
...
<main class="lg:ml-52 min-h-screen p-4 lg:p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-bold text-[#1C1C2E]">Data Gejala</h1>
            <p class="text-xs text-[#9B9AB0] mt-0.5">Kelola data gejala penyakit ikan patin</p>
        </div>
        <button onclick="openModal('modalTambah'); getKodeOtomatis('gejala', 'inputKodeGejala')" class="btn-accent flex items-center gap-2">
            <?php icon('add') ?>
            Tambah
        </button>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-[#E8E7E1] overflow-hidden">
        <div class="flex items-center justify-between px-5 py-4 border-b border-[#F0EFE9]">
            <p class="text-sm font-semibold text-[#1C1C2E]">Total: <span class="text-[#E07A10]"><?= $jumlah_gejala ?></span> gejala</p>
            <form method="GET" class="flex items-center gap-2">
                <div class="relative">
                    ...
                    <input type="text" name="q" value="<?= htmlspecialchars($search) ?>" placeholder="Cari gejala..." class="input-base pl-9 py-2 w-full text-xs">
                </div>
                <button type="submit" class="btn-accent py-2 px-4 text-xs">Cari</button>
                <?php if ($search): ?><a href="gejala.php" class="btn-ghost py-2 px-3 text-xs">Reset</a><?php endif; ?>
            </form>
        </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="text-[10px] uppercase tracking-wide text-[#9B9AB0] bg-[#FAFAF8]">
                        ...
                        <th class="text-left px-5 py-3 font-semibold">Nama Gejala</th>
                ...
                <?php else: while ($r = $rows->fetch_assoc()): ?>
                    <tr class="hover:bg-[#FAFAF8] transition-colors fade-in">
                        <td class="px-5 py-3 font-bold text-[#E07A10] text-xs"><?= htmlspecialchars($r['kode_gejala']) ?></td>
                        <td class="px-5 py-3 font-semibold text-[#1C1C2E]"><?= htmlspecialchars($r['nama_gejala']) ?></td>
                        <td class="px-5 py-3 text-xs text-muted"><?= date('d/m/Y', strtotime($r['created_at'])) ?></td>
                        <td class="px-5 py-3">
                            <div class="flex items-center justify-center gap-1.5">
                                <button onclick='openEdit(<?= json_encode($r) ?>)' class="w-7 h-7 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-600 flex items-center justify-center transition-colors" title="Edit">
                                ...
                                <button onclick="confirmHapus(<?= $r['id'] ?>, '<?= addslashes($r['nama_gejala']) ?>')" class="w-7 h-7 rounded-lg bg-red-50 hover:bg-red-100 text-red-500 flex items-center justify-center transition-colors" title="Hapus">
...
<div id="modalTambah" class="modal-backdrop">
    <div class="modal-card">
        <div class="flex items-center justify-between mb-5">
            <div><h3 class="font-bold text-[#1C1C2E]">Tambah Gejala</h3><p class="text-xs text-[#9B9AB0] mt-0.5">Isi data gejala baru</p></div>
            <button onclick="closeModal('modalTambah')"><?php icon('close') ?></button>
        </div>
        <form method="POST" class="space-y-3">
            <input type="hidden" name="action" value="tambah">
            <div>
                <label class="block text-xs font-semibold text-[#9B9AB0] mb-1">Kode Gejala</label>
                <input type="text" name="kode" id="inputKodeGejala" required class="input-base">
            </div>
            <div>
                <label class="block text-xs font-semibold text-[#9B9AB0] mb-1">Nama Gejala</label>
                <input type="text" name="nama" placeholder="Nama gejala yang terlihat" required class="input-base">
            </div>
...
<div id="modalEdit" class="modal-backdrop">
    <div class="modal-card">
        <div class="flex items-center justify-between mb-5">
            <div><h3 class="font-bold text-[#1C1C2E]">Edit Gejala</h3><p class="text-xs text-[#9B9AB0] mt-0.5">Perbarui data gejala</p></div>
        ...
        <form method="POST" class="space-y-3">
            ...
            <div>
                <label class="block text-xs font-semibold text-[#9B9AB0] mb-1">Kode Gejala</label>
                <input type="text" name="kode" id="editKode" required class="input-base">
            </div>
            <div>
                <label class="block text-xs font-semibold text-[#9B9AB0] mb-1">Nama Gejala</label>
                <input type="text" name="nama" id="editNama" required class="input-base">
            </div>
            <div class="flex gap-2 pt-2">
...
<div id="modalHapus" class="modal-backdrop">
    <div class="modal-card max-w-sm text-center">
        ...
        <h3 class="font-bold text-[#1C1C2E] mb-1">Hapus Gejala?</h3>
        <p class="text-xs text-[#9B9AB0] mb-5">Gejala <strong id="hapusNama" class="text-[#1C1C2E]"></strong> akan dihapus permanen.</p>
...
function openEdit(d) {
    ...
    document.getElementById('editKode').value = d.kode_gejala;
    document.getElementById('editNama').value = d.nama_gejala;
    openModal('modalEdit');
}
